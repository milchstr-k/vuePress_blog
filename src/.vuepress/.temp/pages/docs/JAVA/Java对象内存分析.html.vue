<template><div><h1 id="java对象" tabindex="-1"><a class="header-anchor" href="#java对象" aria-hidden="true">#</a> Java对象</h1>
<blockquote>
<p>📌 <strong>关键词：</strong> java、内存</p>
</blockquote>
<h2 id="对象构成" tabindex="-1"><a class="header-anchor" href="#对象构成" aria-hidden="true">#</a> 对象构成</h2>
<ul>
<li>对象头
<ul>
...
<li>1 bit: boolean</li>
<li>1 byte: byte</li>
<li>2 byte: short|char</li>
<li>4 byte: int|float|oops(Ordinary Object Pointers)</li>
<li>8 byte: long|double</li>
</ul>
</li>
<li>对齐填充
<ul>
<li>非必需，保证对象大小是 8 字节的整数倍进行填充的长度。</li>
<li>仅占位</li>
</ul>
</li>
</ul>
<figure><img src="@source/docs/JAVA/images/2023-08-03-13-44-45.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<p><img src="@source/docs/JAVA/images/2023-08-03-13-45-11.png" alt="" loading="lazy"><br>
<img src="@source/docs/JAVA/images/2023-08-03-13-45-27.png" alt="" loading="lazy"></p>
<p>在不同位数的JVM中长度不一样，但是基本组成内容是一致的。</p>
<ul>
...
<h3 id="class-pointer" tabindex="-1"><a class="header-anchor" href="#class-pointer" aria-hidden="true">#</a> Class Pointer</h3>
<p>类型指针，是对象指向它的类元数据的指针，虚拟机通过这个指针来确定这个对象是哪个类的实例。</p>
<h3 id="实例数据" tabindex="-1"><a class="header-anchor" href="#实例数据" aria-hidden="true">#</a> 实例数据</h3>
<p>对象有属性字段，则会有数据信息，无属性字段则无数据信息。</p>
<h2 id="数据对齐" tabindex="-1"><a class="header-anchor" href="#数据对齐" aria-hidden="true">#</a> 数据对齐</h2>
<p>数据对齐非必须。默认情况下，Java虚拟机堆中对象的起始地址需要对齐至8的倍数。如果一个对象用不到8N个字节则需要对其填充，以此来补齐对象头和实例数据占用内存之后剩余的空间大小。如果对象头和实例数据已经占满了JVM所分配的内存空间，那么就不用再进行对齐填充了。</p>
<h3 id="原因" tabindex="-1"><a class="header-anchor" href="#原因" aria-hidden="true">#</a> 原因</h3>
<p>字段内存对齐其中的一个原因，是让字段只出现在同一CPU的缓存行中。如果字段不是对齐的，那么就有可能出现跨缓存行的字段。也就是说，该字段的读取可能需要替换两个缓存行，而该字段的存储也会同时污染两个缓存行。这两种情况对程序的执行效率而言都是不利的。对齐填充最终目的是为了计算机高效寻址。</p>
<h3 id="对象分析" tabindex="-1"><a class="header-anchor" href="#对象分析" aria-hidden="true">#</a> 对象分析</h3>
<h4 id="maven引入jol-core" tabindex="-1"><a class="header-anchor" href="#maven引入jol-core" aria-hidden="true">#</a> Maven引入jol-core</h4>
